...
# # Isolation Forest 모델 생성 및 파라미터 조정
...
def update(frame, X, clf_svm, clf_if, pca, scaler, ax1, ax2):
    ...
    prediction_svm = (prediction_svm == 1).astype(int)  # -1을 0으로, 1을 1로 변환
    ...
    prediction_if = (prediction_if == 1).astype(int)  # -1을 0으로, 1을 1로 변환
